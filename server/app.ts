...
// Keep a lightweight API welcome response during local backend development.
if (process.env.NODE_ENV !== 'production') {
  app.get('/', (_req: Request, res: Response) => {
    res.status(200).json({
      message: "Jisr Hyperlocal Marketplace API'sine Hoş Geldiniz.",
      docs: '/api/health',
      version: '1.0.0',
    });
  });
...
// The production Node process serves the compiled Vite client and lets React
// Router handle all non-API routes. This keeps deep links deployable from one
// service without affecting local Vite development.
if (process.env.NODE_ENV === 'production') {
