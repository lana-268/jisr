...
export function Toggle({ checked, onChange, disabled = false, label, description, compact = false }: ToggleProps) {
  return <div className={`flex items-center justify-between gap-4 ${compact ? '' : 'rounded-xl border border-border bg-surface p-4'}`}>
    ...
    <button type="button" role="switch" aria-checked={checked} aria-label={label} disabled={disabled} onClick={() => onChange(!checked)} className="flex h-11 w-14 shrink-0 items-center justify-center rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
      <span className={`relative h-7 w-12 rounded-full transition-colors ${checked ? 'bg-success' : 'bg-border-strong'}`}>
