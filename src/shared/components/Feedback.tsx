...
export function Toast({ message, tone = 'success', onClose }: ToastProps) { if (!message) return null; return <div aria-live="polite" className={`fixed bottom-5 right-4 z-[60] flex max-w-sm items-center gap-3 rounded-xl border bg-surface py-2 ps-4 pe-2 text-sm font-medium shadow-popover ${tone === 'error' ? 'border-red-200 text-danger' : 'border-border text-heading'}`}><span className={`h-2 w-2 shrink-0 rounded-full ${tone === 'error' ? 'bg-danger' : 'bg-success'}`} /><span className="flex-1">{message}</span><button type="button" aria-label="Dismiss notification" onClick={onClose} className="icon-button shrink-0"><X className="h-4 w-4" /></button></div>; }
