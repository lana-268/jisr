...
export function CustomerMessages({ onMessageSent }: CustomerMessagesProps) {
  ...
  return <div className="-m-5 flex h-[min(680px,78vh)] overflow-hidden sm:-m-6" aria-label="Customer messages">
    ...
    {selected ? <section className={`${showThread ? 'flex' : 'hidden md:flex'} min-w-0 flex-1 flex-col bg-surface`} aria-label={`Conversation about ${selected.itemTitle}`}>
      ...
      <form onSubmit={sendMessage} className="flex items-end gap-2 border-t border-border p-3 sm:p-4"><label htmlFor="customer-message-draft" className="sr-only">Message your provider</label><textarea id="customer-message-draft" rows={1} value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Write a message…" className="min-h-11 flex-1 resize-none rounded-xl border border-border-strong px-3.5 py-2.5 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary-soft"/><Button type="submit" disabled={!draft.trim()} className="!h-11 !w-11 !min-h-11 !px-0" aria-label="Send message" icon={<Send className="h-4 w-4"/>}/></form>
    </section> : null}
  </div>;
}