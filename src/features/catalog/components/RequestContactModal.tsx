...
import { saveCustomerRequest } from '../data/customerRequests';
...
export function RequestContactModal({ entry, onClose, onSuccess }: RequestContactModalProps) {
  ...
  const submit = (event: React.FormEvent) => { event.preventDefault(); setSubmitted(true); if (!note.trim() || !date || !time) return; setSaving(true); window.setTimeout(() => { saveCustomerRequest(entry, note.trim(), date, time); setSaving(false); onClose(); onSuccess('Demo request created. Backend connection will be added later.'); }, 450); };
