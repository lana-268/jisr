...
export function AuthPage() {
  ...
  return (
    <main className="grid min-h-[100dvh] bg-page lg:grid-cols-2">
      <section className="flex items-center justify-center px-4 py-8 sm:px-8">
        <div className="w-full max-w-md">
          <PublicBrand />
          <div className="mt-10">
            <p className="text-xs font-bold uppercase tracking-[.16em] text-primary">
              {register ? "Join the neighborhood" : "Welcome back"}
            </p>
            <h1 className="mt-3 text-3xl font-bold text-heading sm:text-4xl">
              {register ? "Create your Jisr account" : "Log in to Jisr"}
            </h1>
            <p className="mt-2 text-sm leading-6 text-muted">
              {register
                ? "Choose how you want to take part in your local marketplace."
          ...
          {register && (
            <div className="mt-7 grid grid-cols-1 gap-3 sm:grid-cols-3">
              ...
              <button
                ...
                className={`min-h-24 rounded-xl border p-4 text-start ${role === "admin" ? "border-primary bg-primary-soft" : "border-border bg-surface"}`}
              >
          ...
          <form noValidate onSubmit={submit} className="mt-7 space-y-5">
            {register && (
              <Input
                ...
                error={
                  submitted && !name.trim() ? "Enter your name." : undefined
                }
              />
            )}
            <Input
            ...
            >
              {register ? "Create demo account" : "Log in"}
            </Button>
          </form>
          <p className="mt-6 text-center text-sm text-muted">
            {register ? "Already have an account?" : "New to Jisr?"}{" "}
            <Link
          ...
          <p className="mt-4 text-center text-xs text-muted">
            Demo only—no credentials are stored or transmitted.
          </p>
        </div>
      </section>
      <aside className="relative hidden overflow-hidden bg-sage-soft lg:block">
        <img
          src="https://images.unsplash.com/photo-1529156069898-49953e39b3ac?auto=format&fit=crop&w=1200&q=85"
          alt="Neighbors sharing time together"
          className="h-full w-full object-cover"
        />
        <div className="absolute inset-0 bg-heading/25" />
        <div className="absolute bottom-10 start-10 end-10 rounded-2xl bg-surface/95 p-6">
          <p className="text-xl font-semibold text-heading">
            People make places feel like home.
          </p>
          <p className="mt-2 text-sm text-muted">
            Connect with trusted skills, food, and help in your neighborhood.
          </p>
        </div>
      </aside>
    </main>
