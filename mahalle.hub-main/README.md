# 🏙️ MahalleHub Backend API

Production-ready, modular Node.js backend built with **Express**, **TypeScript**, and **Firebase Admin SDK** for **MahalleHub** — a hyperlocal marketplace for neighborhood services and home-cooked products in Turkey.
...
- **Framework**: Express.js with modular 3-tier layering:
  - `src/routes`: Route handlers & path definitions
  - `src/controllers`: Request validation & HTTP response orchestration
  - `src/services`: Business logic & Firestore operations
  - `src/middlewares`: Firebase ID token authentication, role-based guards, global error handling
  - `src/types`: Strict String Union Types (zero regular TypeScript enums)
  - `src/config`: Firebase Admin SDK initialization with live/emulator/fallback dual-mode
...
# 2. Seed database with Istanbul neighborhood demo data
npm run seed
