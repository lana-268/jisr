...
export const authenticate = async (
  ...
  try {
    ...
    if (auth) {
      ...
      // If live Firebase auth is not configured but a generic token was sent
      // Treat token as user UID for hackathon testing
      const profile = await resolveUserProfile(token);
      req.user = {
        uid: token,
        role: profile?.role || 'CUSTOMER',
        ...(profile?.adminType ? { adminType: profile.adminType } : {}),
      };
      return next();
    }
  } catch (error: any) {
